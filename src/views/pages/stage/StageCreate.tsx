...
import { CategoryModel, FormStageModel, FormStageValidations, RequirementModel } from "@/models";
...
export const StageCreate = (props: createProps) => {
  ...
  const {
    name, start, end, weighing, requirements,
    onInputChange, isFormValid, onValueChange, onResetForm,
    nameValid, startValid, endValid, weighingValid, requirementsValid } = useForm(item ?? formFields, formValidations);
    ...
    <>
    {
      ...
      >
        <RequirementTable
          stateSelect={true}
          itemSelect={(v) => {
            if (requirements.map((e: RequirementModel) => e.id).includes(v.id)) {
              onValueChange('requirements', [...requirements.filter((e: RequirementModel) => e.id != v.id)])
            } else {
          ...
          <DialogContent sx={{ display: 'flex' }}>
            ...
            <Grid item xs={12} sm={12} sx={{ padding: '5px' }}>
              <ComponentSelect
                ...
                items={requirements.map((e: RequirementModel) => ({ id: e.id, name: e.name }))}
                onRemove={(v) => onValueChange('requirements', [...requirements.filter((e: RequirementModel) => e.id != v)])}
              />
            </Grid>
          </DialogContent>
          <DialogActions>
